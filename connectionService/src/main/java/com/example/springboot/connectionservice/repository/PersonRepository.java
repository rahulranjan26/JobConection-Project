package com.example.springboot.connectionservice.repository;
...
public interface PersonRepository extends Neo4jRepository<Person, UUID> {
    ...
    @Query("""
                MATCH (p:Person) -[:CONNECTED_TO*2]-> (q)
                where p.userId <> q.userId and p.userId=$userId
                return q
            """)
    ...
    @Query("MATCH (p1:Person)-[r:REQUESTED_BY]->(p2:Person) " +
